...
namespace API.Controllers
{
    ...
    public class AdminController : ControllerBase
    {
        ...
        public async Task<IActionResult> ApprovePayment([FromBody] vm_PaymentApproveRequest req)
        {
            ...
            // ✅ req.FarmerId and req.Amount are now populated from DB inside the method above

            if (success)
            {
                await _rabbitMqService.PublishToUserAsync(req.FarmerId,
                    "Payment Approved 💰",
                    $"Your final 70% payment of ₹{req.Amount:N2} has been approved. UTR: {generatedUtr}",
                    "payment");

                await _rabbitMqService.PublishToRoleAsync("admin",
                    "Final Payment Released",
                    $"70% payment of ₹{req.Amount:N2} has been released to farmer.",
                    "payment");

                return Ok(new { success = true, message = "Payment successfully released." });
            }
        ...
        public async Task<IActionResult> UpdateStatus([FromBody] vm_FarmerStatusRequest request)
        {
            ...
            if (success)
            {
                // ✅ NOTIFICATION: Farmer status updated
                // Changed from request.FarmerId to request.UserId
                await _rabbitMqService.PublishToUserAsync(request.UserId,
                    $"Account {(request.Status ? "Activated" : "Deactivated")}",
                    $"Your farmer account has been {(request.Status ? "activated" : "deactivated")} by admin.",
                    "account_status");
            }
            ...
            if (status == 1)
            {
                ...
                if (!string.IsNullOrEmpty(email))
                {
                    try
                    {
                        ...
                        // ✅ NOTIFICATION: Vendor approved
                        await _rabbitMqService.PublishToRoleAsync("vendor",
                            "Account Approved ✅",
                            $"Your vendor account for {businessName} has been approved by admin. You can now start ordering.",
                            "account_status");
                    }
        ...
        public async Task<IActionResult> Create([FromBody] CreateFieldOfficerViewModel model)
        {
            ...
            // ✅ NOTIFICATION: New Field Officer created
            await _rabbitMqService.PublishToRoleAsync("admin",
                "New Field Officer Added",
                $"New field officer '{model.FullName}' has been added to the system.",
                "field_officer");
        ...
        public async Task<IActionResult> ToggleFOStatus([FromForm] int userId, [FromForm] bool status)
        {
            ...
            if (!string.IsNullOrEmpty(email))
            {
                ...
                // ✅ NOTIFICATION: FO Status changed
                await _rabbitMqService.PublishToUserAsync(userId,
                    status ? "Account Activated ✅" : "Account Deactivated ❌",
                    status ? "Your field officer account has been activated by admin." : "Your field officer account has been deactivated by admin.",
                    "account_status");
            }
        ...
        public async Task<IActionResult> UpdateStatus(string id, [FromBody] FoStatusPatchRequest request)
        {
            ...
            try
            {
                ...
                if (!string.IsNullOrEmpty(email))
                {
                    await _emailService.SendFieldOfficerStatusChangedEmailAsync(email, name, "Assigned Region", request.IsActive);

                    // ✅ NOTIFICATION: FO Status changed
                    await _rabbitMqService.PublishToUserAsync(Convert.ToInt32(id),
        ...
        public async Task<IActionResult> UpdateVendorStatus(string vendorId, [FromQuery] string status, [FromBody] AdminActionRequest request)
        {
            ...
            // ✅ NOTIFICATION: Vendor status updated
            await _rabbitMqService.PublishToRoleAsync("vendor",
                $"Account {status}",
                $"Your vendor account has been {status.ToLower()} by admin. Reason: {request.Reason}",
                "account_status");
        ...
        public async Task<IActionResult> CreateWarehouse([FromBody] WarehouseCreateRequest request)
        {
            ...
            // ✅ NOTIFICATION: New warehouse created
            await _rabbitMqService.PublishToRoleAsync("admin",
                "New Warehouse Created",
                $"New warehouse '{request.Name}' has been added to the system.",
                "warehouse");
        ...
        public async Task<IActionResult> UpdateWarehouse(long id, [FromBody] WarehouseUpdateRequest request)
        {
            ...
            // ✅ NOTIFICATION: Warehouse updated
            await _rabbitMqService.PublishToRoleAsync("admin",
                "Warehouse Updated",
                $"Warehouse '{request.Name}' has been updated.",
                "warehouse");
        ...
        public async Task<IActionResult> ToggleStatus(long id, [FromQuery] bool isActive)
        {
            ...
            // ✅ NOTIFICATION: Warehouse status toggled
            await _rabbitMqService.PublishToRoleAsync("admin",
                $"Warehouse {(isActive ? "Activated" : "Deactivated")}",
                $"Warehouse has been {(isActive ? "activated" : "deactivated")}.",
                "warehouse");
